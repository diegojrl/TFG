...
public class Database {
    ...
    public static boolean insertOpinion(String sourceId, String targetId, float opinion) throws SQLException {
        PreparedStatement stm = Instance.db.prepareStatement("SELECT OPINION FROM OPINIONS WHERE SOURCEID = ? AND TARGETID = ?");
        ...
        if (rs.next()) {
            ...
            if (opinion ==  oldOpinion) {
                ...
                stm.setFloat(1, opinion);
                stm.setString(2, sourceId);
                stm.setString(3, targetId);
                return stm.executeUpdate() == 1;
            }
        } else {
            stm = Instance.db.prepareStatement("INSERT INTO OPINIONS(SOURCEID, TARGETID, OPINION) VALUES (?, ?, ?)");
            stm.setString(1, sourceId);
            stm.setString(2, targetId);
            stm.setFloat(3, opinion);
            return stm.executeUpdate() == 1;
        }
    }
